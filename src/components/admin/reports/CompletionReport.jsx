...
export default function CompletionReport() {
  ...
  async function load() {
    setLoading(true);
    try {
      const result = await apiFetch(`${ADMIN}/reports/completion`);
      if (result?.success) setRows(result.payload?.completions || []);
    } catch (e) {
      // silently fail
    } finally {
      setLoading(false);
  ...
  const years = useMemo(
    () =>
      [...new Set(rows.map((r) => r.completedAt && new Date(r.completedAt).getFullYear()))]
        .filter(Boolean)
        .sort((a, b) => b - a),
    [rows]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      const matchesYear =
        yearFilter === 'all' ||
        (r.completedAt && new Date(r.completedAt).getFullYear() === Number(yearFilter));
  ...
  const summary = [
    { label: 'Completions',  value: filtered.length,                                          color: 'text-zinc-900' },
    { label: 'Certificates', value: filtered.filter((r) => r.certificateIssued).length,        color: 'text-emerald-600' },
    { label: 'Pending',      value: filtered.filter((r) => !r.certificateIssued).length,       color: 'text-amber-600' },
    { label: 'This year',    value: filtered.filter((r) => r.completedAt && new Date(r.completedAt).getFullYear() === new Date().getFullYear()).length, color: 'text-blue-600' },
  ];
  ...
  async function handleExport() {
    ...
    try {
      ...
      const response = await fetch(`${ADMIN}/reports/completion/export?${params}`, {
      ...
      a.href = url;
      a.download = `completion_report_${Date.now()}.csv`;
      a.click();
  ...
  return (
    <ReportLayout
      ...
      description="Learners who have finished their programmes and received certification."
      ...
      <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            <FaCertificate className="text-zinc-300 text-2xl mx-auto mb-3" />
            <h3 className="font-bold text-zinc-900 mb-1">No completions yet</h3>
            <p className="text-sm text-zinc-500">
              {hasFilters
                ? 'Try adjusting your filters.'
                : 'Once learners complete their programmes, they will appear here.'}
            </p>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-zinc-50 border-b border-zinc-200">
                  <tr>
                    {['Learner', 'Programme', 'Cohort', 'Completed', 'Certificate', 'Issued', 'Actions'].map((h) => (
                    ...
                    <tr key={r.id || i} className="border-b border-zinc-100 last:border-0 hover:bg-zinc-50">
                      ...
                      <td className="px-4 py-3 text-zinc-600 whitespace-nowrap">
                        {r.certificateIssuedAt
                          ? new Date(r.certificateIssuedAt).toLocaleDateString('en-ZA', {
                              day: 'numeric', month: 'short', year: 'numeric',
                            })
                          : '—'}
                      </td>
