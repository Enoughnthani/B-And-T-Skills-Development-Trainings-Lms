...
const OUTCOME_STYLES = {
  ...
  RETAKE:  'bg-purple-100 text-purple-700',
};
...
export default function AssessmentReport() {
  ...
  async function load() {
    setLoading(true);
    try {
      const result = await apiFetch(`${ADMIN}/reports/assessments`);
      if (result?.success) {
        setRows(result.payload?.assessments || []);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      const matchesSearch =
        ...
        r.assessmentName?.toLowerCase().includes(s);
      ...
      return matchesSearch && matchesProgramme && matchesOutcome;
    });
  }, [rows, search, programmeFilter, outcomeFilter]);
  ...
  const passCount = filtered.filter((r) => r.outcome === 'PASS').length;
  const totalGraded = filtered.filter((r) => ['PASS', 'FAIL'].includes(r.outcome)).length;
  ...
  const summary = [
    ...
    { label: 'Failed',    value: filtered.filter((r) => r.outcome === 'FAIL').length,  color: 'text-red-600' },
    { label: 'Pass rate', value: `${passRate}%`,                                       color: 'text-blue-600' },
  ];

  const hasFilters =
    search || programmeFilter !== 'all' || outcomeFilter !== 'all';
  ...
  async function handleExport() {
    ...
    try {
      ...
      const response = await fetch(`${ADMIN}/reports/assessments/export?${params}`, {
      ...
      a.href = url;
      a.download = `assessment_report_${Date.now()}.csv`;
      a.click();
      ...
      <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            <FaClipboardCheck className="text-zinc-300 text-2xl mx-auto mb-3" />
            <h3 className="font-bold text-zinc-900 mb-1">No assessments found</h3>
            <p className="text-sm text-zinc-500">
              {hasFilters
                ? 'Try adjusting your filters.'
                : 'Once assessments are graded, they will appear here.'}
            </p>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-zinc-50 border-b border-zinc-200">
                  <tr>
                    {['Learner', 'Assessment', 'Programme', 'Attempts', 'Score', 'Outcome', 'Assessor', 'Date'].map((h) => (
                ...
                <tbody>
                  {paginated.map((r, i) => (
                    <tr key={r.id || i} className="border-b border-zinc-100 last:border-0 hover:bg-zinc-50">
                      ...
                      <td className="px-4 py-3 text-zinc-600 whitespace-nowrap">{r.assessmentName}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider whitespace-nowrap ${
                          OUTCOME_STYLES[r.outcome] || 'bg-zinc-100 text-zinc-600'
                        }`}>
                          {r.outcome || 'PENDING'}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-zinc-600 whitespace-nowrap">{r.assessor || '—'}</td>
                      <td className="px-4 py-3 text-xs text-zinc-500 whitespace-nowrap">
                        {r.gradedAt
                          ? new Date(r.gradedAt).toLocaleDateString('en-ZA', {
